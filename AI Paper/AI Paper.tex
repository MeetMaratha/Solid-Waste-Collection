...
Experiments were carried out on a Ryzen 5 2500U processor which runs at 2GHz and utilizes 16 GB RAM. The model was implemented in Python 3.9.6 and solved with Gurobi optimizer version Gurobi 8.0.1. A total of four scenarios were implemented and the minimum and maximum solving time  was around 1.4 s having total variable count: 28706 and maximum linear constraint: 37042 for the maximum case.
...
Often the decision makers want to derive the requirement of resources that could cater to the whole demand. To achieve that, we relaxed the constraint on available resources to deduce the the total resource requirement for achieving 100\% bin visits, with high waste. In the previous case it was established that six trucks would cater to all the bins for region1 and region3. We extended the experiment for region2 by increasing the available trucks till we achieved 100\% bin coverage. It was observed that region2 was fully covered seven trucks (see Table \ref{tab3}). Figure \ref{fig3} shows the final calculated routes for each trucks of regions.
...
We can observe that for region 2, the total coverage doesn't have the same rise in as compared to regions 1 and 3. It is because, region 2 is larger than region 1 and 3, and contains more nodes. Thus, the observation is explained by the fact that in order to have the same coverage with a higher number of nodes, it would need a larger number of trucks as well. And since, we initially took a uniform number of trucks per region, as a result, the coverage in region 2 is slightly less than the other regions. As expected, once more trucks are added for region 2, it catches up to the other regions in terms of bin coverage.
\subsubsection*{Case 3: Weight Sensitivity Analysis}
In order to depict the effects of different weights for the objectives on the waste collected and the distance travelled by the collection trucks, we have done a detailed sensitivity analysis using the case of 3 trucks per region as the base. This is also beneficial in providing a range of options for policy-makers to choose from, depending upon regional requirements and which objective they wish to prioritize.

We observe a range of values for both waste (\ref{figs1}) and distance (\ref{figs2}) for different weights from which appropriate weights may be selected for the desired outcome. In the depiction, only $w_1$ is used as summation of the weights is equal to 1 \eqref{eq1.5}. Thus, the results for $w_2$ will be identical but mirrored. For the calculations in this paper, the values of 0.9 and 0.1 have been used for $w_1$ and $w_2$ respectively, because those values were found to be the ideal compromise between weight and distance objectives in the current scenario.
\begin{figure}[H]
    \centering
    \begin{subfigure}{0.5\textwidth}
        \centering
    ...
    \label{figsz}
\end{figure}
To get a closer and more detailed look at the effect of weights on both of the objectives, we have Figure \ref{figs}, which was formed by normalizing the raw results from the sensitivity analysis to the same scale, followed by plotting the normalized data against the different weights. This is conducive to finding patterns and selecting the best weights required for any case.
\begin{figure}[H]
    \centering
