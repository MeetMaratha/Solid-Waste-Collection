...
Waste collection is one of the most common yet one of the most integral systems present in all modern societies, helping maintain a sanitary environment across the urban landscape.The existing waste collection systems face huge challenges, and are even going to increase in the near future as the global waste generation is set to double by 2030 \cite{hoornweg2012waste}, hence an urgent attention to waste planning is the key to develop sustainable cities.
Waste collection; however, in the cities of developing nations has been largely overlooked, primarily due to resource constraints. Waste collection vehicle is one such resource, whose route planning is often not driven by analytics driven conclusions.  
\begin{itemize}
\color{red}
\item In a study done about the costs of municipal waste collection for the city of Kragujevac, the annual costs of waste collection amount to nearly 14000 euros, which accounts for about 70\% of the total municipal solid waste costs \cite{boskovic2016calculating}. That was the case for a city with a population of around one hundred and fifty thousand people. 

\end{itemize}

Vehicle route planning is the key to a good waste collection system requires modeling a lot of components such as path optimization, consideration of available resources, spatiotemporal patterns, and real-time dynamics of waste volume at collection points and trucks. A plethora of literature is present that addresses subsets of these components in the solution development. However, a holistic waste collection system that considers them simultaneously for any region has not been reported in the literature. Hence, the onground implementation of the present approaches is still very limited. This has significant impact on the operation costs and on eventually on the environment. Moreover, the components and their interrelationships are very complex for the resource constrained societies, and therefore pose altogether new challenges, which requires urgent attention of the researchers.

\begin{itemize}
\color{blue}
\item One of the major resource constraints which is a backbone of a waste management system is strategic routing of available waste collection vehicles.
