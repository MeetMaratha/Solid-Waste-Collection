...
\begin{table}[H]\label{variables}
	...
	\begin{tabular*}{486pt}[H]{|c|c|}
		...
		\hline \multirow{2}{*}{$C_{ij} \in [0, 1]$} & Distance cost from a node i\\
		...
		& visiting node i.\\
		\hline \multirow{2}{*}{$u_{i}  \in [0, P_{t}] $} & The percentage fill of the truck at node i \\
		...
		\hline \multirow{2}{*}{$BT \in \mathbb{Z}$} & The conversion factor for fill \\
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{Static_Weighted_3_Truck.png} % Image changed here
    \caption{Routes for 1 truck per region as calculated by static optimization}\label{fig4}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{Dynamic_weighted_3_Truck.png} % Image changed here
