...
We address the problem of developing a vehicle routing model by minimizing the incurred travel cost and maximizing the waste collected by the vehicles considering various real-time considerations. We have considered an Indian city to demonstrate its outcomes. To implement the model we have divided the study area in various regions. The region consists of collection bins which are generally used to collect waste from a larger areas using a road network made from edges and nodes. We simulated temporally varying waste values for each of these bins that can be one of the nodes, we call them "smart bins" which are implemented using priority queues based on the distance to that bins and the amount of waste value. The simulation helped us in overcoming the requirement of physical sensor placement at these locations. We incorporated the inputs of stakeholder agency in the simulation purpose to keep the simulation values closer to the actual scenarios. The "route" in our problem corresponds to path of the vehicles covering these bins. To make the implementation realistic we have applied limits on the total waste that can be collected by the vehicles.
The model is executed after a certain time interval, That considers bins values, current vehicle location during the time interval and generates a route which keeps updating with the time. The model stops until happens repeatedly until either the run is complete or the vehicle is full. The "run" in our problem signifies a collection trip for one vehicle, beginning and ending at the depot. The fact that the values of the bins can updated even when the vehicle is on the move, makes it a realistic case. To implement such conditions the model assumes that the vehicle has completed its current edge and considers the next node as the starting point of the new route calculation. 
...
The objective of the model is to maximize the waste collected while minimizing the total distance travelled by the vehicles. We formulate the model as a mixed integer linear programming problem and solved it using Gurobi optimizer (http://www.gurobi.com/) in polynomial time. Such solvers can solve problems with realistic instance sizes, which was in our case.
...
The following table defines all the variables used in mathematical formulation
\begin{table}[H]\label{variables}
	...
	\begin{tabular*}{486pt}[H]{|c|c|}
		...
		\hline \multirow{1}{*}{st} & The starting node of the truck in that run.\\
		...
		\hline \multirow{1}{*}{$P_{t}$} & Fill percentage of truck at time t.\\
		\hline
	\end{tabular*}
\end{table} 
